...
def get_error(result):
    #1行をスキップして0~16列までのデータをdataへ格納する
    data2 = np.loadtxt(result, delimiter=',',skiprows=1,usecols=(29,33,37,41,45,49,53))
...
def plot_value8(d, q, dist, n, def_ang, f_dir, fname, dd, qid, dd2, dd3, dd5, e):
	#出力値を定義
	...
	#--------------------------------------------------------
	#グラフを定義
	plt.figure(figsize=(90.0,45.0))	#figsize=（横幅、縦幅）
	...
	#類似度グラフ
	plt.subplot(2,3,2)#グラフを描画する場所を定義
	...
	plt.plot(sim, linestyle='solid',lw=0.2,  color ='blue', label="simirality")
	#plt.scatter(s_sim)
	plt.legend(loc="lower right")
	#plt.ylim(0,0.2)
	...
	plt.text(0,0.6, str(maxsd),size=10,color="black") #推定位置座標
	#------------------------------------------------------------------

	#UMapを与えた存在確率分布
	plt.subplot(2,3,4)#グラフを描画する場所を定義
	plt.title("normal distribution and UMap" ,color="black")
	plt.xlabel('norm')
	plt.ylabel('probability')
	plt.text(0,0.6, str(maxdu),size=10,color="black") #推定位置座標
	...
	plt.axvline(norm[ti], ls = "--", color = "gold")
	plt.plot(norm[maxsdui], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='cyan', alpha=0.3)
	#plt.plot(norm, du, linestyle='solid',lw=0.1,  color ='cyan', label="query_pixel")
	plt.plot(norm, s_du, linestyle='solid',lw=1.0,  color ='cyan', label="simirality")
	...
	#偏差値を与えた存在確率分布を作成
	plt.subplot(2,3,5)#グラフを描画する場所を定義
	...
	plt.axvline(norm[ti], ls = "--", color = "gold")
	plt.plot(norm[maxsdevii], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='green', alpha=0.3)
	#maxd3 = dd3.argmax()
	plt.text(0,0.1, str(maxdevi),color="black") #推定位置座標
	plt.plot(norm, s_devi, linestyle='solid',lw=1.0,  color ='green', label="simirality")
	...
	plt.subplot(2,3,6)#グラフを描画する場所を定義
	plt.title('Hotelling')
	plt.xlabel('norm')
	plt.ylabel('probability')
	plt.text(0,0.6, str(maxhote),color="black") #推定位置座標
	#plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	#plt.axvline(norm[maxssi], ls = "--", color = "blue")
	#plt.axvline(norm[maxshotei], ls = "--", color = "red")
	plt.axvline(e[0,5], ls = "--", color = "red")
	#plt.axvline(norm[ti], ls = "--", color = "gold")
	#plt.plot(norm[maxshotei], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='red', alpha=0.3)
	plt.plot(norm, s_hote, linestyle='solid',lw=0.01,  color ='red', label="simirality")
	...
	# plt.plot(norm, s_sim, linestyle='solid',lw=1.0,  color ='powderblue', label="simirality")
	# plt.plot(norm, s_dist, linestyle='solid',lw=1.0,  color ='grey', label="simirality")
	# plt.plot(norm, s_du, linestyle='solid',lw=1.0,  color ='lavenderblush', label="simirality")
	...
	# plt.plot(norm, s_sim, linestyle='solid',lw=1.0,  color ='blue', label="simirality")
	# plt.plot(norm, s_devi, linestyle='solid',lw=1.0,  color ='green', label="simirality")
	...
	# plt.plot(norm[maxssi], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='red', alpha=0.3)
	# plt.plot(norm, s_sim, linestyle='solid',lw=1.0,  color ='blue', label="simirality")
	# plt.plot(norm, s_hote, linestyle='solid',lw=1.0,  color ='red', label="simirality")
...
def main():
		...
		while(query_number != qnum):
			...
			plot_value8(data, query, dist, norm, def_ang, f_dir, fname, dist_data, query_id, dist2_data, dist3_data, dist5_data, error_data)
