...
def plot_value7(d, q, dist, n, def_ang, f_dir, fname, dd, qid, dd2, dd3):
	Pbst = n.argmin()#normの最小値
	A = d[:,6]
	B = d[:,7]
	Bc= d[:,8]
	...
	#類似度グラフ
	plt.subplot(5,1,2)#グラフを描画する場所を定義
...
#	plt.text(0,0.0, "d:"+str(round(dist[p],4))+", n:"+str(round(n[p],3)),size=12,color="blue")
...
def main():
		...
		while(query_number != qnum):
			...
			plot_value7(data, query, dist, norm, def_ang, f_dir, fname, dist_data, query_id, dist2_data, dist3_data)
