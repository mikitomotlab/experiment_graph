...
def get_GT_position(f_dir, query_id, res_data, n):
	...
	x, y, z, th, ph = res_data[n, 1], res_data[n, 2], 0.25, res_data[n, 3] , 0
...
def calc_distance(q, db):
	...
	print("db[i,1] : {}".format(db[0,1]))
	print("q[0] : {}".format(db[0,2]))
	print("db[i,4] : {}".format(db[24764,4]))
	...
	for i, dummy in enumerate(db[:,0]):
		...
		c = np.array([db[i,1],db[i,2],db[i,4]*np.pi/180])
		d = np.array([q[0],q[1],q[3]])
...
def get_error(result):
    #1行をスキップして0~16列までのデータをdataへ格納する
    data2 = np.loadtxt(result, delimiter=',',skiprows=1,usecols=(29,33,36,41,45,49,53))

    return data2

def get_error2(distname):
    #1行をスキップして0~16列までのデータをdataへ格納する
    data = np.loadtxt(distname, delimiter=',',skiprows=1,usecols=(28,32,36,40,44,48,52))
...
def normgraph(norm, value):
	...
	maxmyy = max(myy)
	return myx, myy, myymax, maxi


def plot_value8(d, q, dist, n, def_ang, f_dir, fname, dd, qid, dd2, dd3, dd5, e, qnum):
	#出力値を定義
	...
	print(len(s_dist))
	print(norm[0])
	print("maxshote : {}".format(maxshote))
	...
	#--------------------------------------------------------
	#グラフを定義
	plt.figure(figsize=(30,10))	#figsize=（横幅、縦幅）unit: インチ
	...
	# #類似度グラフ
	# plt.subplot(1,1,1)#グラフを描画する場所を定義
	...
	# # #plt.axvline(e[0,0], ls = "--", color = "blue")
	...
	# plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="simirality")
	# # --------------------------------------------------------------
	...
	plt.xticks(np.arange(0, 16 + 1, 1))
	plt.plot(norm, s_dist, linestyle='solid',lw=0.1,  color ='grey', label="simirality")
	#plt.scatter(norm, s_sim)
	plt.axvline(e[qnum,0], ls = "--", color = "blue")
	plt.axvline(e[qnum,2], ls = "--", color = "grey")
	...
	plt.text(0,0.51, str(e[qnum,2]),size=10,color="black") #推定位置座標
	# #------------------------------------------------------------------
	...
	# plt.text(0,0.5, str(e[qnum,3]),size=10,color="black") #推定位置座標
	# #plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	# plt.xticks(np.arange(0, 16 + 1, 1))
	# plt.axvline(e[qnum,0], ls = "--", color = "blue")
	# plt.axvline(e[qnum,3], ls = "--", color = "skyblue")
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	# # plt.plot(norm, s_sim, linestyle='solid',lw=0.01,  color ='blue', label="simirality")
	# plt.plot(norm, s_du, linestyle='solid',lw=0.1,  color ='skyblue', label="simirality")
	# # plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="simirality")
	...
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	# plt.axvline(e[qnum,0], ls = "--", color = "blue")
	# plt.axvline(e[qnum,4], ls = "--", color = "green") #偏差値の推定位置
	# # plt.text(0,0.5, str(e[qnum,4]),color="black") #推定位置座標
	# plt.plot(norm, s_devi, linestyle='solid',lw=0.1,  color ='green', label="simirality")
	# # plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="simirality")
	# #------------------------------------------------------------------
	...
	# plt.xlabel('norm')
	# plt.ylabel('probability')
	# plt.text(0,50, str(e[qnum,5]),color="black") #推定位置座標
	# plt.xticks(np.arange(0, 16 + 1, 1))
	# #plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	# plt.axvline(e[qnum,0], ls = "--", color = "blue")
	# # plt.axvline(e[qnum,2], ls = "--", color = "grey")
	# plt.axvline(e[qnum,5], ls = "--", color = "red")
	# plt.axvline(e[qnum,6], ls = "--", color = "black") #ホテリングの推定位置
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	# plt.plot(norm, s_hote, linestyle='solid',lw=0.1,  color ='red', label="simirality")
	# # plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="UMap")
	# #-----------------------------------------------------
	...
	# plt.text(0,0.6, str(e[qnum,3]),color="black") #推定位置座標
	# #plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	# plt.xticks(np.arange(0, 16 + 1, 1))
	# plt.axvline(e[qnum,0], ls = "--", color = "blue")
	# plt.axvline(e[qnum,3], ls = "--", color = "skyblue")
	# plt.axvline(e[qnum,2], ls = "--", color = "grey") #ホテリングの推定位置
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	# #plt.plot(norm[maxssi], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='red', alpha=0.3)
	# plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="UMap")
	# plt.plot(norm, s_dist, linestyle='solid',lw=0.1,  color ='grey', label="odometry")
	# plt.plot(norm, s_du, linestyle='solid',lw=0.1,  color ='skyblue', label="UMapdistribution")
	# #-----------------------------------------------------
	...
	# plt.xlabel('norm')
	# plt.ylabel('probability')
	# plt.text(0,0.6, str(e[qnum,3]),color="blue") #推定位置座標
	# #plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	# plt.xticks(np.arange(0, 16 + 1, 1))
	# plt.axvline(e[0,0], ls = "--", color = "blue")
	# plt.axvline(e[0,3], ls = "--", color = "green")
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	# #plt.plot(norm[maxssi], maxsd, 'o', markersize=8, markeredgewidth=1.89, color='none', markeredgecolor='blue', alpha=0.3)
	# plt.plot(norm, s_sim, linestyle='solid',lw=0.01,  color ='blue', label="simirality")
	# plt.plot(norm, s_dist, linestyle='solid',lw=0.01,  color ='green', label="simirality")
	# #-----------------------------------------------------
	...
	# plt.xlabel('norm')
	# plt.ylabel('probability')
	# plt.text(0,0.6, str(e[qnum,5]),color="black") #推定位置座標
	# plt.xticks(np.arange(min(norm), max(norm)+1, 1.0))
	# plt.axvline(e[qnum,0], ls = "--", color = "blue")
	# plt.axvline(e[qnum,2], ls = "--", color = "grey")
	# plt.axvline(e[qnum,3], ls = "--", color = "skyblue")
	...
	# plt.plot(norm, s_sim, linestyle='solid',lw=0.1,  color ='blue', label="simirality")
	# plt.plot(norm, s_hote, linestyle='solid',lw=0.1,  color ='red', label="simirality")
	# # print("norm[maxshotei] : {}".format(norm[maxshotei]))
	# #-----------------------------------------------------
	...
	# plt.plot(norm, s_hd, linestyle='solid',lw=1.0,  color ='powderblue', label="simirality")
	# plt.axvline(norm[ti], ls = "--", color = "gold")
	...
	# plt.text(0,0.6, str(maxhote),color="blue") #推定位置座標
	# plt.plot(norm, s_hd, linestyle='solid',lw=1.0,  color ='powderblue', label="simirality")
	# plt.plot(norm, s_hu, linestyle='solid',lw=1.0,  color ='lavenderblush', label="simirality")
...
def main():
		...
		while(query_number != qnum):
			...
			error_data = get_error2(resultsname)

			plot_value8(data, query, dist, norm, def_ang, f_dir, fname, dist_data, query_id, dist2_data, dist3_data, dist5_data, error_data, qnum)
